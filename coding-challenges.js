...
// HECK YEAH I got it into one line
function cubeSum (arr) { return arr.map(val => Math.pow(val, 3)).reduce((a,b) => a+b) }
...
function oneMind(arr1, arr2) {
    let together = arr1.concat(arr2)
    let unique = []
    together.map(x => { if(!unique.includes(x)) { unique.push(x) } })
    return unique
};

console.log(oneMind(arr1,arr2));